...
public class HibernateDao<T, PK extends Serializable> extends
	...
	@SuppressWarnings("unchecked")
	public List findPageForExport(final String hql,
			final Object... values) {
		// Assert.notNull(page, "page不能为空");
...
//		if(fromHql.indexOf("group by")>0){
//			// select子句与group by子句会影响count查询,进行简单的排除.
//			fromHql = "from " + StringUtils.substringAfter(fromHql, "from");
//			fromHql = StringUtils.substringBefore(fromHql, "group by");
//		}else 
	
			// select子句与order by子句会影响count查询,进行简单的排除.(经验证，mysql不会影响)
			fromHql = "from " + StringUtils.substringAfter(fromHql, "from");
